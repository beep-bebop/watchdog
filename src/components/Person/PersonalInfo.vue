...
export default {
	...
	computed:{
		tableData1: function(){
			return store.getters["video/getPersonalTab"];
		}
	},
    methods: {
      handleEdit(index, row) {
		...
		if(index == 0){
			...
			this.$prompt('请输入新的电话号码：', '提示', {
			    	  ...
			    	  store.commit('video/setPersonalPhone',value);
			          this.$message({
			          type: 'success',
			          message: ' 修改成功，记得保存哟~ '
			        });
			...
			this.$prompt('请输入新的所属单位：', '提示', {
			          confirmButtonText: '确定',
			          cancelButtonText: '取消',
			    }).then(({ value }) => {
					  store.commit('video/setPersonalCom',value);
			          this.$message({
			          type: 'success',
			          message: ' 修改成功，记得保存哟~ '
			        });
			...
			this.$prompt('请输入新的宿舍楼号：', '提示', {
					  ...
					  store.commit('video/setPersonalSsl',value);
			          this.$message({
			          type: 'success',
			          message: ' 修改成功，记得保存哟~ '
			        });
			...
			this.$prompt('请输入新的宿舍号：', '提示', {
			          confirmButtonText: '确定',
			          cancelButtonText: '取消',
			    }).then(({ value }) => {
					  store.commit('video/setPersonalSsh',value);
			          this.$message({
			          type: 'success',
			          message: ' 修改成功，记得保存哟~ '
			        });
